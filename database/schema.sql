...
CREATE TABLE IF NOT EXISTS `tramite_seguimiento` (
    ...
    `estado_anterior` ENUM('REGISTRADO','EN_FIRMAS','PRESENTADO','OBSERVADO','INSCRITO','CERRADO','ANULADO') NULL,
    `estado_nuevo` ENUM('REGISTRADO','EN_FIRMAS','PRESENTADO','OBSERVADO','INSCRITO','CERRADO','ANULADO') NOT NULL,
    `comentario` TEXT,
    `usuario` VARCHAR(50) NOT NULL DEFAULT 'operador',
    `created_at` TIMESTAMP DEFAULT CURRENT_TIMESTAMP,
    CONSTRAINT `fk_tramite_seguimiento_tramite` FOREIGN KEY (`tramite_id`) REFERENCES `tramites`(`id`) ON DELETE CASCADE
);
